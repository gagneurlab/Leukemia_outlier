...
COSMIC_EXOME = os.path.join(os.environ['INTOGEN_DATASETS'], 'deconstructsigs', 'signatures.cosmic.exome.tsv')


# run functions

@click.command()
@click.option('--input-file', type=click.Path(exists=True))  # weights from deconstructSigs
@click.option('--output-file', type=click.Path())
def run_assign(input_file, output_file):
    ...
    results_new.to_csv(output_file, sep='\t')

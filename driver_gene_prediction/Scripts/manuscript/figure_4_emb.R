...
#'    - wBhtml: '`sm config["htmlOutputPath"] + "/manuscript/figure_4_emb.html"`'
#'  type: noindex
#'  resources:
#'    - mem_mb: 8000 
#' output:   
...
saveRDS(snakemake, file.path(snakemake@params$projectPath,
                             "/processed_data/snakemake/figure_4_emb.snakemake"))
# snakemake <- readRDS("/s/project/vale/driver_prediction_202402/processed_data/snakemake/figure_4_emb.snakemake")
...
heatmap_path <- "/s/project/vale/Resource/figure_4_emb_heatmap.rds"
...
pdf(paste0(output_dir, "/figure_4_emb_top100.pdf"), width = 10, height = 14)
...
png(paste0(output_dir, "/figure_4_emb_top100.png"), width = 10, height = 14, units = "in", res = 600)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   p_c, 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))
...
saveRDS(p_c, snakemake@output$p_c_emb)
saveRDS(p_d, snakemake@output$p_d_emb)
...
### arrange plot - heatmap #### 
pdf(paste0(output_dir, "/figure_4_emb_same_gene.pdf"), width = 10, height = 14)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))
...
png(paste0(output_dir, "/figure_4_emb_same_gene.png"), width = 10, height = 14, units = "in", res = 600)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))

...
saveRDS(snakemake, file.path(snakemake@params$projectPath,
                             "/processed_data/snakemake/outrider_tab.snakemake"))
# snakemake <- readRDS("/s/project/vale/driver_prediction_202304/processed_data/snakemake/outrider_tab.snakemake")
...
createPadjCutoffbyZScore <- function(ods, cutoffs, gene_dt, sample_group){
  ...
  colnames(padj_res_wide)[-1] <- paste(sample_group, "or", colnames(padj_res_wide)[-1], sep="-")
...
createZscoreSig <- function(ods, cutoffs, gene_dt, sample_group){
  ...
  colnames(zScore_dt)[-1] <- paste(sample_group, "or", colnames(zScore_dt)[-1], sep="-")
...
outrider_tab_path <- snakemake@output$outrider_tab
...
outrider_tab <- merge(padjCutoffs, zScore, by='gene_id')
fwrite(outrider_tab, outrider_tab_path) 
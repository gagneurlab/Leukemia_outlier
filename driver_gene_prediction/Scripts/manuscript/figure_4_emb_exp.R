...
#'    - wBhtml: '`sm config["htmlOutputPath"] + "/manuscript/figure_4_emb_exp.html"`'
#'  type: noindex
#'  resources:
#'    - mem_mb: 8000 
#' output:   
...
saveRDS(snakemake, file.path(snakemake@params$projectPath,
                             "/processed_data/snakemake/figure_4_emb_exp.snakemake"))
# snakemake <- readRDS("/s/project/vale/driver_prediction_202402/processed_data/snakemake/figure_4_emb_exp.snakemake")
...
heatmap_path <- "/s/project/vale/Resource/figure_4_emb_exp_heatmap.rds"
...
pdf(paste0(output_dir, "/figure_4_emb_exp_top100.pdf"), width = 10, height = 14)
...
png(paste0(output_dir, "/figure_4_emb_exp_top100.png"), width = 10, height = 14, units = "in", res = 600)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   p_c, 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))
...
saveRDS(p_c, snakemake@output$p_c_emb_exp)
saveRDS(p_d, snakemake@output$p_d_emb_exp)
...
heatmap_path <- "/s/project/vale/Resource/figure_4_emb_exp_heatmap.rds"
phm_omics_emb <- readRDS(heatmap_path)
predicted_100genes_omics_emb <- phm_omics_emb@ht_list$matrix_1@row_names_param$labels

# overlap 64 genes
intersect(predicted_100genes_omics, predicted_100genes_omics_emb) %>% length()

# predicted by omics only
setdiff(predicted_100genes_omics, predicted_100genes_omics_emb) 
# [1] "ANKRD36C" "ARAP3"    "ATXN1"    "ATXN3"    "CALHM3"   "CHD2"     "CHEK2"    "DIS3"     "DNM2"     "EIF3E"    "ENOSF1"  
# [12] "FAM221A"  "FBXL12"   "GNAS"     "HLA-A"    "HLA-B"    "HLA-DRB5" "IGSF3"    "MEGF9"    "MS4A18"   "MYC"      "NBPF11"  
# [23] "PCNT"     "PIGO"     "PTEN"     "REEP4"    "SEL1L3"   "SMG1"     "SORL1"    "SP140"    "TTN"      "UBC"      "USP9Y"   
# [34] "VWF"      "WASHC2C"  "ZNF605"

# predicted after adding emb
setdiff(predicted_100genes_omics_emb, predicted_100genes_omics) 
# [1] "AFF1"    "BCL6"    "BLNK"    "CD79A"   "EBF1"    "ETV6"    "FANCE"   "HOXA9"   "HUS1"    "IKZF3"   "IRF4"    "ITGA2B"  "KIR2DL3"
# [14] "KIR3DL2" "KIR3DL3" "KMT2C"   "KRAS"    "LCK"     "LMO2"    "LYL1"    "NFE2"    "PALB2"   "PBX1"    "PIK3R1"  "PRDM1"   "RB1"    
# [27] "RBBP8"   "SHC1"    "SPIB"    "STAT6"   "TAL1"    "TCF12"   "TCF3"    "TYK2"    "VAV1"    "WT1"    
...
### arrange plot - heatmap #### 
pdf(paste0(output_dir, "/figure_4_emb_exp_same_gene.pdf"), width = 10, height = 14)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))
...
png(paste0(output_dir, "/figure_4_emb_exp_same_gene.png"), width = 10, height = 14, units = "in", res = 600)
p_abc <- ggarrange(ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ggplot() + theme(panel.background = element_blank()), 
                   ncol = 1, labels = c('A', 'B', 'C'), heights = c(1, 1, 0.8))

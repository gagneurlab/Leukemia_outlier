...
# %%
snakemake_path = snakemake.params.projectPath + "/processed_data/snakemake/figure_4_prep_prc.p"
pickle.dump(snakemake, open(snakemake_path, "wb")) 

# %%
# file = open("/s/project/vale/driver_prediction_202402/processed_data/snakemake/figure_4_prep_prc.p",'rb')
...
# %% [markdown]
# # embedding - Precision Recall Curve
...
for exp_viz_index, exp_viz_row in exp_viz.iterrows():
    # read in res
    res = pd.read_csv(exp_viz_row['res_pre_path'], sep='\t')
    input_feature = exp_viz_row['coess_input_feature']
...
# %%
prc_ci_df.to_csv(snakemake.output.mll_emb_prc, sep='\t', index=False)

# %%
performance_df.to_csv(snakemake.output.mll_emb_ap_full, sep='\t', index=False)

# %% [markdown]
# # embedding - Average Precision plot
...
# %%
ap_df.to_csv(snakemake.output.mll_emb_ap, sep='\t', index=False)
...
for exp_viz_index, exp_viz_row in exp_viz.iterrows():
    # read in res
    res = pd.read_csv(exp_viz_row['res_pre_path'], sep='\t')
    input_feature = exp_viz_row['model_method'] 
...
# %%
prc_ci_df.to_csv(snakemake.output.mll_method_prc, sep='\t', index=False)

# %%
performance_df.to_csv(snakemake.output.mll_method_ap_full, sep='\t', index=False)

# %% [markdown]
# # method - Average Precision plot
...
# %%
ap_df.to_csv(snakemake.output.mll_method_ap, sep='\t', index=False)

# %% [markdown]
# # var filter - Precision Recall Curve/Average Precision

# %%
exp_viz_var_filter = pd.read_csv(snakemake.input.exp_viz_var_filter, sep='\t')
exp_viz_var_filter

# %%
res_var_filter = pd.read_csv(snakemake.input.mll_var_filter, sep='\t')
res_var_filter = res_var_filter.assign(Method_Sample_group = res_var_filter.apply(lambda x:'-'.join([x['Method'], x['Sample_group']]), axis=1))
res_var_filter
...
for exp_viz_index, exp_viz_row in exp_viz_var_filter.iterrows():
    # read in res
    res = pd.read_csv(exp_viz_row['res_pre_path'], sep='\t')
    input_feature = exp_viz_row['intogen_input_dir']
...
# %%
prc_ci_df.to_csv(snakemake.output.mll_var_filter_prc, sep='\t', index=False)

# %%
performance_df

# %%
performance_df.to_csv(snakemake.output.mll_var_filter_ap_full, sep='\t', index=False)

# %% [markdown]
# # var filter - Average Precision plot
...
# %%
ap_df.to_csv(snakemake.output.mll_var_filter_ap, sep='\t', index=False)
